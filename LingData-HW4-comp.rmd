...
College - 3, PhDongoing - 1, Postgraduate - 14, UniversityDegree - 12.
5. How many left- and right-randed participants are present?
2 - lefthanded, 28 - righthanded.
...
nrow(distinct(mono_socio, ParticipantID))
mono_socio %>% filter(QuestionCategory == 'sex') %>% count(Responce)
...
```{r}
library(ggplot2)
# either density plot or histogram
ggplot(mono, aes(RT)) + geom_histogram(color = 'white') + xlim(0, 15000)
ggplot(mono, aes(RT)) + geom_density() +  xlim(0, 15000)
```
...
No, we cannot. The long right tail breaks this assumption.
### 1.3
...
```{r}
ggplot(mono, aes(RTlog)) + geom_density() +  xlim(2, 5)
...
In that case we can say distribution is approximately normal with slight positive skewness.
### 1.5
...
```{r}
ggplot(mono1, aes(TypeOfStimuli)) + geom_bar(position = 'dodge', aes(fill=AcceptabilityJ)) + facet_grid(~WordOrder) + theme(text = element_text(size=8), axis.text.x = element_text(angle=45, hjust=1), legend.position = "bottom", legend.background = element_rect(size=0.5, linetype="solid", colour ="black"), axis.title.y = element_blank())
```
...
```{r}
mono1 %>% ggplot(aes(WordOrder, fill=AcceptabilityJ)) + geom_bar(position='dodge') + theme(axis.title.y = element_blank())
...
p <- ggplot(mono1, aes(x=AcceptabilityJ, y=RTlog, fill=WordOrder)) + 
  geom_violin()
p + geom_boxplot(width=0.4, position=position_dodge(0.9))
...
```{r}
library(ggpubr)
ggline(mono1, x = "TypeOfStimuli", y = "RTlog", 
       add = c("mean_se"), color = 'WordOrder', size=0.3) + theme(text = element_text(size=9))
...
```{r}
mean_rtlog_long <- mono1 %>% group_by(ParticipantID, WordOrder) %>%
...
```
Would you reject null hypothesis (under 5% significance level) according to this test? No.

What claim about logarithms of reaction time for Congruent and Incongruent stimuli can you make according to this test? They are similar? While means of logarithms are not similar, they do not differ significantly. And confidence interval of difference crosses zero.
...
```{r}
mean_rtlog <- mean_rtlog %>% mutate(diff = Incongruent - Congruent, base = 0)
t.test(mean_rtlog$diff, mean_rtlog$base, alternative = 'greater')
```

Whould you reject null hypothesis? Yes

What claim about logarithms of reaction time for Congruent and Incongruent stimuli can you make now? In paired test they significantly differ. Mean of difference is 0.016, p-value < 0.01.

How can you interpret difference with the result of 2.3?
Paired test compares subjects at 2 different times. Means in unpaired test were insignificant because of intersubject variability. And we should make paired test, because in data there are 2 conditions of every person.
...
# incong_rtlog <- mono %>% filter(WordOrder == 'Incongruent') %>% group_by(ParticipantID, TypeOfStimuli) %>% summarise(RTlog = mean(log10(RT)))
...
```{r}
aov_res = aov(RTlog ~ TypeOfStimuli, data = incong_rtlog)
summary(aov_res)
```
...
SubjectiveComment-Material and Shape-Color. p-value = 0.015. difference = -0.056.
```{r}
TukeyHSD(aov_res)
```
